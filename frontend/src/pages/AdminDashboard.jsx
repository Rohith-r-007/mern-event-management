...
const AdminDashboard = () => {
    ...
    const [formData, setFormData] = useState({
        title: '', description: '', date: '', location: '', category: '', totalSeats: '', price: '', image: ''
    });

    const resetForm = () => {
        setEditingEventId(null);
        setFormData({ title: '', description: '', date: '', location: '', category: '', totalSeats: '', price: '', image: '' });
    ...
    const handleClearCancelledBookings = async () => {
        if (!window.confirm('Delete all cancelled bookings permanently?')) return;

        try {
            await api.delete('/bookings/cancelled');
            fetchData();
            alert('Cancelled bookings cleared.');
        } catch (error) {
            alert(error.response?.data?.message || 'Error clearing cancelled bookings');
        }
    };

    const handleConfirmBooking = async (id, paymentStatus) => {
        const confirmation = window.confirm(`Are you sure you want to approve this booking as ${paymentStatus === 'paid' ? 'Paid' : 'Pending'}?`);
    ...
    return (
        <div className="max-w-7xl mx-auto">
            <div className="bg-black text-white rounded-2xl p-6 sm:p-8 mb-8 shadow-lg flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
                ...
                >
                    {showEventForm ? (editingEventId ? 'Cancel Edit' : 'Cancel') : '+ Create New Event'}
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        ...
                        <h3 className="text-3xl font-black text-green-600">₹{bookings.reduce((sum, b) => b.paymentStatus === 'paid' && b.status === 'confirmed' ? sum + b.totalPrice : sum, 0)}</h3>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                ...
                                events.map(event => (
                                    <li key={event._id} className="p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-gray-50 transition border-b border-gray-100 last:border-0">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            <button onClick={() => handleEditEvent(event)} className="text-blue-600 hover:text-white hover:bg-blue-600 border border-blue-200 px-4 py-2 rounded-lg text-sm font-bold transition shadow-sm">
                                                Edit
                                            </button>
                                            <button onClick={() => handleDeleteEvent(event._id)} className="text-red-500 hover:text-white hover:bg-red-500 border border-red-200 px-4 py-2 rounded-lg text-sm font-bold transition shadow-sm">
                                                Delete
                                            </button>
                                        </div>
                                    </li>
                ...
                <div className="flex flex-col">
                    <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
                        ...
                        <button onClick={handleClearCancelledBookings} className="bg-red-50 text-red-700 hover:bg-red-500 hover:text-white border border-red-200 text-xs font-bold py-2.5 px-4 rounded-lg transition">
                            Clear Cancelled
                        </button>
                    </div>
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                                    ...
                                    <li key={booking._id} className={`p-6 hover:bg-gray-50 transition border-l-4 ${booking.status === 'pending' ? 'border-l-yellow-400' : booking.status === 'confirmed' ? 'border-l-green-400' : 'border-l-red-400'}`}>
                                        ...
                                        <div className="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-100 text-sm">
                                            ...
                                            <p className="text-gray-700 flex items-center gap-2 mb-1">
                                                ...
                                                <span className={`font-semibold ${booking.totalPrice === 0 ? 'text-green-600' : ''}`}>{booking.totalPrice === 0 ? 'Free' : `₹${booking.totalPrice}`}</span>
                                            </p>
                                            <p className="text-gray-700 flex items-center gap-2 mb-1">
                                                <span className="font-bold w-16 text-gray-500 uppercase text-xs">Date:</span>
                                                <span>{booking.eventId?.date ? new Date(booking.eventId.date).toLocaleDateString() : 'N/A'}</span>
                                            </p>
                                            {booking.eventId && (
                                        ...
                                        {booking.status === 'pending' && (
                                            <div className="flex flex-wrap gap-2 mt-2">
                                                ...
                                                <button onClick={() => handleConfirmBooking(booking._id, 'pending')} className="flex-1 min-w-[120px] bg-gray-50 text-gray-700 hover:bg-gray-800 hover:text-white border border-gray-200 text-xs font-bold py-2.5 px-3 rounded-lg shadow-sm transition">
                                                    ✓ Approve as Pending
                                                </button>
